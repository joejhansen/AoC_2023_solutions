...
pub fn test_values() -> Vec<&'static str> {
    vec![
        "seeds:
    79
    14
...
pub fn map_to_value(value: String, maps: &Vec<&str>) -> String {
    let mut result: &str = "";
    let value_int = value.parse::<u64>().unwrap();
    let mut is_found: bool = false;
    for map in maps {
        let mut map_strs = map.split(" ").collect::<Vec<&str>>();
        map_strs.retain(|value| !value.is_empty());
        let (destination_range, source_range, variance) = (
            map_strs[0].parse::<u64>().unwrap(),
            map_strs[1].parse::<u64>().unwrap(),
            map_strs[2].parse::<u64>().unwrap(),
        );
        if value_int >= source_range && value_int <= source_range + variance {
            let index_to_check = value_int - source_range;
            let destination = destination_range + index_to_check;
...
pub fn p1(values: &Vec<&str>) -> Vec<u64> {
    let mut locations_vec: Vec<u64> = vec![];
    let mut values = values.clone();
    values.rotate_left(1);
    let mut seeds_to_check = values.pop().unwrap().split(":").collect::<Vec<&str>>()[1]
        .split('\n')
        .map(|value| value.trim())
        .collect::<Vec<&str>>();
    seeds_to_check.retain(|value| !value.is_empty());
    let mut humidity_to_location = values.pop().unwrap().split(":").collect::<Vec<&str>>()[1]
        .split('\n')
        .map(|value| value.trim())
        .collect::<Vec<&str>>();
    humidity_to_location.retain(|value| !value.is_empty());
    let mut temperature_to_humidity = values.pop().unwrap().split(":").collect::<Vec<&str>>()[1]
        .split('\n')
        .map(|value| value.trim())
        .collect::<Vec<&str>>();
    temperature_to_humidity.retain(|value| !value.is_empty());
    let mut light_to_temperature = values.pop().unwrap().split(":").collect::<Vec<&str>>()[1]
        .split('\n')
        .map(|value| value.trim())
        .collect::<Vec<&str>>();
    light_to_temperature.retain(|value| !value.is_empty());
    let mut water_to_light = values.pop().unwrap().split(":").collect::<Vec<&str>>()[1]
        .split('\n')
        .map(|value| value.trim())
        .collect::<Vec<&str>>();
    water_to_light.retain(|value| !value.is_empty());
    let mut fertilizer_to_water = values.pop().unwrap().split(":").collect::<Vec<&str>>()[1]
        .split('\n')
        .map(|value| value.trim())
        .collect::<Vec<&str>>();
    fertilizer_to_water.retain(|value| !value.is_empty());
    let mut soil_to_fertilizer = values.pop().unwrap().split(":").collect::<Vec<&str>>()[1]
        .split('\n')
        .map(|value| value.trim())
        .collect::<Vec<&str>>();
    soil_to_fertilizer.retain(|value| !value.is_empty());
    let mut seed_to_soil = values.pop().unwrap().split(":").collect::<Vec<&str>>()[1]
        .split('\n')
        .map(|value| value.trim())
        .collect::<Vec<&str>>();
    seed_to_soil.retain(|value| !value.is_empty());
    for seed in seeds_to_check {
        let soil_num = map_to_value(seed.to_string(), &seed_to_soil);
        let fertilizer_num = map_to_value(soil_num, &soil_to_fertilizer);
        let water_num = map_to_value(fertilizer_num, &fertilizer_to_water);
        let light_num = map_to_value(water_num, &water_to_light);
        let temp_num = map_to_value(light_num, &light_to_temperature);
        let humidity_num = map_to_value(temp_num, &temperature_to_humidity);
        let location_num = map_to_value(humidity_num, &humidity_to_location);
        locations_vec.push(location_num.parse::<u64>().unwrap());
    }
    locations_vec
}

// DO NOT RUN THIS -- IT FROZE MY COMPUTER SOMEHOW AND DON'T WANT TO FIX
// IN RUST, I DON'T KNOW IF THAT'S STUPIDLY IMPRESSIVE OR IMPRESSIVELY STUPID
pub fn p2(values: &Vec<&str>) -> Vec<u64> { 
    let mut locations_vec: Vec<u64> = vec![];
    let mut values = values.clone();
    values.rotate_left(1);
    let mut seed_values_to_check = values.pop().unwrap().split(":").collect::<Vec<&str>>()[1]
        .split('\n')
        .map(|value| value.trim())
        .collect::<Vec<&str>>();
    seed_values_to_check.retain(|value| !value.is_empty());
    let mut seed_starts: Vec<&str> = vec![];
    let mut seed_ranges: Vec<&str> = vec![];
    for i in 0..seed_values_to_check.len() {
        if i % 2 == 0 {
            seed_starts.push(seed_values_to_check[i]);
        } else {
            seed_ranges.push(seed_values_to_check[i]);
        }
    }
    let mut seeds_to_check: Vec<String> = vec![];
    for i in 0..seed_starts.len() {
        let some_vec = (seed_starts[i].parse::<u64>().unwrap()
            ..seed_starts[i].parse::<u64>().unwrap() + seed_ranges[i].parse::<u64>().unwrap())
            .collect::<Vec<u64>>();
        seeds_to_check.extend(some_vec.iter().map(|value| value.to_string()));
    }
    let mut humidity_to_location = values.pop().unwrap().split(":").collect::<Vec<&str>>()[1]
        .split('\n')
        .map(|value| value.trim())
