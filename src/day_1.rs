pub fn all_str_permutations(string: &str) -> (Vec<String>, Vec<String>) {
    let mut new_vector: Vec<String> = vec![];
    let mut new_vector_reverse: Vec<String> = vec![];
    for slice_length in 1..string.len() {
        let mut sample_string: String = String::from("");
        for c in 0..slice_length {
            sample_string.push(string.chars().collect::<Vec<char>>()[c]);
        }
        new_vector.push(sample_string);
        let mut sample_string_reverse: String = String::from("");
        for c in 0..slice_length {
            sample_string_reverse.push(string.chars().rev().collect::<Vec<char>>()[c]);
        }
        new_vector_reverse.push(sample_string_reverse.chars().rev().collect::<String>());
    }
    (new_vector, new_vector_reverse)
}
pub fn p1(values: &str) -> i32 {
    let values_vec: Vec<&str> = values.split("\n").collect();
    ...
    for value in values_vec {
        ...
        calibration_value += number_string.parse::<i32>().unwrap();
    }
    calibration_value
}
pub fn p2(values: &str) -> i32 { // this is borked
    let cool_vec: Vec<&str> = vec![
        "one", "two", "three", "four", "five", "six", "seven", "eight", "nine",
    ];
    let mut partials: Vec<String> = vec![];
    let mut partials_reversed: Vec<String> = vec![];
    for word in &cool_vec {
        let (going_forward, going_backwards) = all_str_permutations(word);
        partials.extend(going_forward);
        partials_reversed.extend(going_backwards);
    }
    fn match_str_to_number(string: &String) -> Option<char> {
        match string.as_str() {
            "one" => Some('1'),
            "two" => Some('2'),
            "three" => Some('3'),
    ...
    for value in values.split("\n").collect::<Vec<&str>>() {
        let mut index_of_first_find = u32::MAX;
        let mut forward_sample_string_slice: String = String::from("");
        let mut backward_sample_string_slice: String = String::from("");
        let mut number_string: String = String::from("");
        for c in value.chars() {
            if c.is_ascii_digit() {
                println!("Match {value} + {c}");
                number_string.push(c);
                index_of_first_find = value.find(c).unwrap() as u32;
                break;
            }
            forward_sample_string_slice.push(c);
            if cool_vec.contains(&forward_sample_string_slice.as_str()) {
                println!("Match {value} + {forward_sample_string_slice}");
                index_of_first_find = value.find(&forward_sample_string_slice).unwrap() as u32;
                number_string.push(match_str_to_number(&forward_sample_string_slice).unwrap());
                break;
            }
            if partials.contains(&forward_sample_string_slice) {
                continue;
            }
            forward_sample_string_slice = String::from("");
        }
        let reversed_value = value.chars().rev();
        for c in reversed_value {
            if c.is_ascii_digit() {
                if index_of_first_find == value.find(c).unwrap() as u32{
                    break;
                }
                println!("Match {value} - {c}");
                number_string.push(c);
            ...
            backward_sample_string_slice.push(c);
            let flipped_string = backward_sample_string_slice
                .chars()
                .rev()
                .collect::<String>();
            if cool_vec.contains(&flipped_string.as_str()) {
                if index_of_first_find == value.find(&forward_sample_string_slice).unwrap() as u32{
                    break;
                }
                number_string.push(match_str_to_number(&flipped_string).unwrap());
                println!("Match {value} - {flipped_string}");
                break;
            }
            if partials_reversed.contains(&flipped_string) {
                continue;
            }
            backward_sample_string_slice = String::from("");
        }
        let possible_value = number_string.parse::<i32>();
...
pub fn values() -> &'static str {
    let values = "sdpgz3five4seven6fiveh
    876mbxbrntsfm
